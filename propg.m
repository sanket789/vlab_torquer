...
w = x(5:7);
% user input
B_orb = [0 0.3 1]'; %magnetic filed vector assumed get it from user in body frame 
                        % to do get this value using lat-long
I = 5e-3;   % current in amps 

MI = 0.15; % moment of inertia 1/6M*L*L with M=10kg and L = 0.3
N = 100;   % number of turns in each torquer N = 100;
W = [ 0 w(3) -w(2) w(1);
      -w(3) 0 w(1) w(2);
      w(2) -w(1) 0 w(3);
...
tau_b = N*I*[ B_b(3)-B_b(2); B_b(1)-B_b(3); B_b(2)-B_b(1) ]; %calculate tau in orbit frame for cubic geomtry
